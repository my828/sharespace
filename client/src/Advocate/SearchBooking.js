...
// // configure algolia
// const algolia = algoliasearch("UI4XF6GAZS", "d0d9a91e7c68fb44a286ce841f14333d");
...
class Search extends React.Component {
  constructor(props) {
    ...
    let now = new Date()
     let data = [
       {
         key: 0,
         email: "min2222@gmail.com",
         end: now.setDate(now.getDate() + 5),
         ...
         listingData: {
           ...
           availability: [
             {
               ...
               id: "-Lg_MXVCuQdKIGs09mJe",
               listingID: "-Lg_MV42WDEl575AdIBC",
               pushKey: "-Lg_MXV7XDSZ_Xm0-Hja",
               start: new Date(),
               state: "available",
             },
           ],
           ...
           id: "-Lg_MV42WDEl575AdIBC",
           information: "chck in on time plzz",
           location: "Belltown",
           name: "Listing A",
         ...
         start: new Date(),
         state: "available",
         story: "I open my doors to those in need",
       },
     ];
    this.state = {
      view: "list",
  ...
  componentDidMount() {
    this.props.firebase.auth.onAuthStateChanged((user) => {
      if (user) {
        const availabilitiesRef = this.props.firebase
          .availabilities()
          .orderByChild("state")
          .equalTo("available")
          .limitToLast(5);
        ...
        let listingQuery = this.props.firebase
          .availabilities()
          .orderByChild("state")
          .equalTo("available")
          .limitToLast(5);
  ...
  render() {
    ...
    console.log(this.state.allAvail)
        ...
        <div
              ...
              {this.state.view == "list" ? (
                <div className={classes.cardContainer}>
                  {(this.state.allAvail === undefined || this.state.allAvail.length === 0) && (
                    <p style={{ paddingLeft: "30px" }}>No result</p>
                  )}
                 {this.state.allAvail !== undefined && 
                 this.state.allAvail.map((booking) => {
...
const SearchBookings = compose(withStyles(styles), withFirebase)(Search);
